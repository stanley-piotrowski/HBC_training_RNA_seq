...
# Re-impot file for later analysis
counts_data <- read_csv("./data/counts_data.csv")
```
...
# Read in csv for later analysis
counts_metadata <- read_csv("./metadata/counts_metadata.csv")
```
...
* We can see additional patterns arise when we plot different PC axes.  Notably, when we plot PC1 and PC3, the MOV10 knockdown and control samples cluster together.  Interestingly, there is some odd behavior with one of the MOV10 over-expression samples on PC3 that may need to be investigated further.ß
...
* DESeq2 models counts of genes using the negative binomial distribution and generalized linear models (GMLs).  

* It's important to note that there are two types of distributions for modeling count data: the Poisson distribution, and the negative binomial distribution.  The former assumes the variance and mean are equal, and is thus only controlled by a single parameter, lambda; the latter does not make this assumption, and is more flexible for dealing with gene count data.  
* This is critical, because often, the number of technical replicates is quite low for RNA-seq experiments, making it challenging to accurately estimate variance.  

* At a high-level overview, however, what we're aiming to do with DESeq2 is statistically evaluate if the mean expression of different genes across sample groups is different.  The `DESeq()` function estimates the size factors and dispersion of each gene (the dispersion is a metric of the spread of the data distribution), and fits a negative binomial generalized linear model.  The Wald test is conducted to test for statistical significance of each gene.

* The design formula allows us to define which sources of variation we want to control for in the model that could be confounding factors (e.g., sex, or age), and which is the factor of interest (e.g., treatment).

* In the design formula, all factors that we want to account for in the metadata should be included if they are major sources of variation, and the last factor will be the condition of interest.  

#### Exercises

1) _Suppose you wanted to study the expression differences between the two age groups in the metadata shown in the tutorial, and major sources of varition were `sex` and `treatment`, how would the design formula be written?_

* The design formula would be written as: `design <- ~ sex + treatment`.

2) _Based on our Mov10 metadata data frame, which factors should we include in our design formula?_

* Based on the previously conducted QC PCA and hierarchical clustering analyses, there are no additional terms in the metadata data frame that we should include in the design formula other than `sample_type`.  

* Thus, our design formula would written as: `design <- ~ sample_type`.

3) _What would you do if you wanted to include a factor in your design formula that is not in your metadata?_

* If you wanted to do this, you would need to create a new column in your metadata data frame and ensure that it matches the factor in the design formula.  

#### MOV10 DE Analysis

* To run the DESeq analysis, we just need to create a new `DEseq2Dataset` object with the raw counts, and then run the `DESeq()` function.

```{r DESeq}
# Create a new DESeq2Dataset object
dds <- DESeqDataSetFromTximport(txi, colData = metadata, design = ~ sample_type)
...
Below are the various design formulas that would be used to test for different effects while controlling for various factors.

* Test the effect of `treatment`: `design <- ~ treatment`.

* Test for the effect of genotype, while regressing out the variation due to `treatment`: `design <- ~ treatment + genotype.`

* Test for the effect of `genotype` on the `treatment` effects: `design <- ~ genotype + treatment + genotype:treatment`.  

### 5. Hypothesis testing and multiple testing correction

* The next part of the analysis involves fitting the count data to the negative binomial, which models the raw counts of the gene as a function of the mean counts (normalized counts scaled by the sample-specific normalization factor; see above section of the median of ratios), and one dispersion term (RNA-seq data exhibit over-dispersion, or variance greater than the mean).

* The negative binomial model is part of the Generalized Linear Model (GLM) family, whic generalizes linear regression models to allow for error distributions other than the normal distribution, and modeling the response variable as a function of the predictors using a link function.

#### Hypothesis Testing

* The resulting output from the `DESeq2` model is log2 fold changes.

* The Wald test is used for hypothesis testing, the null hypothesis being the log2 fold change is 0, the alternative hypothesis being the log2 fold change is not zero.

* The Wald test takes the log2 fold change and divides it by the associated standard error to create a z-statistic, which is then compared to a standard normal distribution (i.e., normal distribution with a mean of 0 and standard deviation of 1) to calculate a p-value.

* Importantly, the Wald test is used when we are comparing two classes (e.g., treatment versus control), but the likelihood ration test can also be used as a hypothesis test when comparing more than two classes.

* The likelihood ration test extends the capability of the hypothesis test by fitting two models per gene (while the Wald test fits one model per gene); a reduced model with the main factor in the design formula removed; and a full model, with the main factor in the design formula included.  In this case, we are comparing whether there is a significant difference in the fit between the full model and the reduced model.

#### Exercise

1) An appropriate hypothesis test to evaluate differences in gene expression across developmental stages would be the likelihood ratio test, since the ultimate aim is to compare gene expression across multiple time points.

2) Below is the line of code to create the `dds` object.  Here, we would be primarily interested in understanding the interaction of sex and developmental stage on gene expression differences.  

```{r create-dds-ex}
# Code to create dds object
DESeqDataSetFromTximport(txi, colData, 
                         design = "~ sex + dev_stage + sex:dev_stage")

```

3) Below is the line of code used to run `DESeq2` with these data.  The full model would be the full design formula, where we are evaluating gene expression as a function of sex, developmental stage, and the interaction between sex and developmental stage (i.e., allowing male and female mice to have different gene expression profiles over time).  The reduced model would be just evaluating gene expression as a function of sex and developmental stage, essentially assuming that there are no differences in expression with sex.

```{r run-deseq-ex}
# Code to run DESeq
DESeq(mouse_data, test = "LRT", 
      reduced = ~ sex + dev_stage)
```

4) In this case, we could subset the `dds` object and perform a Wald test only comparing these two timepoints.  
