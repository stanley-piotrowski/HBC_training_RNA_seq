...
* __Quality control of aligned sequence reads (STAR/Qualimap)__
...
* __Quality control: aggregating results with MultiQC__
...
* __Different gene expression analysis__
...
Now that we've downloaded all of the Salmon output files and the annotation file, we need to load the libraries from CRAN and Bioconductor.  Note, any Bioconductor packages need to be installed using the command `Biocmanager::install(<packagename>)`. 

```{r load-additional-libraries, warning = FALSE, message = FALSE}
# Load additional libraries from CRAN and Bioconductor
library(DESeq2)
